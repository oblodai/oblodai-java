...
/**
 * Who pays the network fee of a payout.
 *
 * <p>An open vocabulary. The values below are the ones this contract snapshot knows, and they are
 * interned: {@code status == FeeBearer.RECIPIENT} works. A value the
 ...
 */
public final class FeeBearer implements Vocabulary {

    /** {@code recipient} */
    public static final FeeBearer RECIPIENT = new FeeBearer("recipient");

    /** {@code merchant} */
    public static final FeeBearer MERCHANT = new FeeBearer("merchant");

    private static final Map<String, FeeBearer> KNOWN = new LinkedHashMap<>();

    /** Every value this snapshot knows, in the gateway's own order. */
    public static final List<FeeBearer> VALUES = List.of(RECIPIENT, MERCHANT);

    static {
        for (FeeBearer value : VALUES) KNOWN.put(value.wire, value);
    }

    private final String wire;

    private FeeBearer(String wire) {
        this.wire = wire;
    }
     ...
     */
    @JsonCreator
    public static FeeBearer of(String wire) {
        if (wire == null) return null;
        FeeBearer known = KNOWN.get(wire);
        return known != null ? known : new FeeBearer(wire);
    }
     ...
     * @return the value
     */
    public static FeeBearer from(String wire) {
        return of(wire);
    }

    @Override
    public boolean equals(Object other) {
        return other instanceof FeeBearer value && value.wire.equals(wire);
    }
