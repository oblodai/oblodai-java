 ...
 * interned: {@code status == BatchOnError.CONTINUE} works. A value the
 ...
 */
public final class BatchOnError implements Vocabulary {
    ...
    private static final Map<String, BatchOnError> KNOWN = new LinkedHashMap<>();

    /** Every value this snapshot knows, in the gateway's own order. */
    public static final List<BatchOnError> VALUES = List.of(CONTINUE, STOP);

    static {
        for (BatchOnError value : VALUES) KNOWN.put(value.wire, value);
    }

    private final String wire;

    private BatchOnError(String wire) {
        this.wire = wire;
    }
     ...
     */
    @JsonCreator
    public static BatchOnError of(String wire) {
        if (wire == null) return null;
        BatchOnError known = KNOWN.get(wire);
        return known != null ? known : new BatchOnError(wire);
    }
    ...
    public boolean equals(Object other) {
        return other instanceof BatchOnError value && value.wire.equals(wire);
    }
