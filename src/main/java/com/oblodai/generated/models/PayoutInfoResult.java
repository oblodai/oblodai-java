// Generated by oblodai-backend tools/sdkgen from openapi.json - do not edit.

package com.oblodai.generated.models;

import java.math.BigDecimal;
import java.util.LinkedHashMap;
import java.util.LinkedHashSet;
import java.util.Map;
import java.util.Objects;
import java.util.Set;

/**
 * Build one with {@link #builder()}; fields the SDK does not know yet are kept in {@link #extra()}
 * and sent back as they came.
 */
public final class PayoutInfoResult implements WireObject {

    private static final Set<String> FIELDS = Set.of(
            "address",
            "amount",
            "approval_required",
            "commission",
            "created_at",
            "currency",
            "document_url",
            "error",
            "error_code",
            "fee_bearer",
            "is_final",
            "is_refund",
            "memo",
            "network",
            "order_id",
            "payer_amount",
            "payment_order_id",
            "refund_for",
            "source",
            "status",
            "txid",
            "updated_at",
            "uuid");

    private final String address;
    private final BigDecimal amount;
    private final Boolean approvalRequired;
    private final BigDecimal commission;
    private final String createdAt;
    private final String currency;
    private final String documentUrl;
    private final String error;
    private final String errorCode;
    private final PayoutFeeBearer feeBearer;
    private final Boolean isFinal;
    private final Boolean isRefund;
    private final String memo;
    private final String network;
    private final String orderId;
    private final BigDecimal payerAmount;
    private final String paymentOrderId;
    private final String refundFor;
    private final PayoutSource source;
    private final PayoutStatus status;
    private final String txid;
    private final String updatedAt;
    private final String uuid;
    private final Set<String> nulls;
    private final Map<String, Object> extra;

    private PayoutInfoResult(Builder builder) {
        this.address = builder.address;
        this.amount = builder.amount;
        this.approvalRequired = builder.approvalRequired;
        this.commission = builder.commission;
        this.createdAt = builder.createdAt;
        this.currency = builder.currency;
        this.documentUrl = builder.documentUrl;
        this.error = builder.error;
        this.errorCode = builder.errorCode;
        this.feeBearer = builder.feeBearer;
        this.isFinal = builder.isFinal;
        this.isRefund = builder.isRefund;
        this.memo = builder.memo;
        this.network = builder.network;
        this.orderId = builder.orderId;
        this.payerAmount = builder.payerAmount;
        this.paymentOrderId = builder.paymentOrderId;
        this.refundFor = builder.refundFor;
        this.source = builder.source;
        this.status = builder.status;
        this.txid = builder.txid;
        this.updatedAt = builder.updatedAt;
        this.uuid = builder.uuid;
        this.nulls = Set.copyOf(builder.nulls);
        this.extra = Wire.copyMap(builder.extra);
    }

    /**
     * Адрес получателя.
     *
     * @return the {@code address} field
     */
    public String address() {
        return address;
    }

    /**
     * Сумма выплаты в валюте currency, списанная с вашего баланса.
     *
     * @return the {@code amount} field
     */
    public BigDecimal amount() {
        return amount;
    }

    /**
     * true — выплата ждёт подтверждения (внутренние сценарии; по API-ключу всегда false).
     *
     * @return the {@code approval_required} field
     */
    public Boolean approvalRequired() {
        return approvalRequired;
    }

    /**
     * Удержанная сетевая комиссия, в валюте выплаты. 0 — комиссию поглотил шлюз.
     *
     * @return the {@code commission} field
     */
    public BigDecimal commission() {
        return commission;
    }

    /**
     * Время создания (ISO 8601).
     *
     * @return the {@code created_at} field
     */
    public String createdAt() {
        return createdAt;
    }

    /**
     * Код валюты выплаты.
     *
     * @return the {@code currency} field
     */
    public String currency() {
        return currency;
    }

    /**
     * Подписанная ссылка на PDF-чек этой операции — открывается без API-ключа, можно вложить в
     * письмо или отдать получателю. Пусто, если генерация документов не включена.
     *
     * @return the {@code document_url} field
     */
    public String documentUrl() {
        return documentUrl;
    }

    /**
     * Причина сбоя выплаты человеческим текстом; null — сбоя нет.
     *
     * @return the {@code error} field, or {@code null} when absent
     */
    public String error() {
        return error;
    }

    /**
     * Машинный код причины; null — сбоя нет.
     *
     * @return the {@code error_code} field, or {@code null} when absent
     */
    public String errorCode() {
        return errorCode;
    }

    /**
     * Кто заплатил сетевую комиссию: gateway — шлюз поглотил её (commission = 0); merchant — сумма
     * списания увеличена на комиссию, получатель получает запрошенное целиком (is_subtract=true,
     * выплатная ссылка с fee_bearer=merchant); recipient — комиссия удержана из выплаты, получателю
     * приходит меньше запрошенного.
     *
     * @return the {@code fee_bearer} field
     */
    public PayoutFeeBearer feeBearer() {
        return feeBearer;
    }

    /**
     * true — статус финальный (confirmed / failed / cancelled).
     *
     * @return the {@code is_final} field
     */
    public Boolean isFinal() {
        return isFinal;
    }

    /**
     * true — это возврат платежа, а не обычная выплата.
     *
     * @return the {@code is_refund} field
     */
    public Boolean isRefund() {
        return isRefund;
    }

    /**
     * Тег/мемо назначения, переданный при создании (TON Jetton, memo-биржи). Пусто — без мемо.
     *
     * @return the {@code memo} field
     */
    public String memo() {
        return memo;
    }

    /**
     * Сеть блокчейна.
     *
     * @return the {@code network} field
     */
    public String network() {
        return network;
    }

    /**
     * Ваш номер (reference) выплаты. У возврата — null: возврат не имеет вашего идентификатора, см.
     * payment_order_id.
     *
     * @return the {@code order_id} field, or {@code null} when absent
     */
    public String orderId() {
        return orderId;
    }

    /**
     * Сколько реально уходит получателю на адрес: amount − commission.
     *
     * @return the {@code payer_amount} field
     */
    public BigDecimal payerAmount() {
        return payerAmount;
    }

    /**
     * Ваш order_id платежа, по которому сделан возврат (null у обычной выплаты). У возврата
     * собственного order_id нет — он приходит null, а сверять возврат с заказом нужно по этому
     * полю.
     *
     * @return the {@code payment_order_id} field, or {@code null} when absent
     */
    public String paymentOrderId() {
        return paymentOrderId;
    }

    /**
     * Идентификатор возвращаемого платежа (null, если это не возврат).
     *
     * @return the {@code refund_for} field, or {@code null} when absent
     */
    public String refundFor() {
        return refundFor;
    }

    /**
     * api (через интеграцию) | manual (из кабинета).
     *
     * @return the {@code source} field
     */
    public PayoutSource source() {
        return source;
    }

    /**
     * Статус выплаты: pending (создана, ждёт) | approved (одобрена) | awaiting_cosign (ждёт второй
     * подписи) | broadcasting (отправляется) | sent (отправлена, ждёт подтверждений) | confirmed
     * (подтверждена — готово) | failed | cancelled. Значение можно передать обратно в фильтр
     * истории как есть.
     *
     * @return the {@code status} field
     */
    public PayoutStatus status() {
        return status;
    }

    /**
     * Хеш транзакции в блокчейне (появляется после отправки).
     *
     * @return the {@code txid} field
     */
    public String txid() {
        return txid;
    }

    /**
     * Время последнего изменения (ISO 8601).
     *
     * @return the {@code updated_at} field
     */
    public String updatedAt() {
        return updatedAt;
    }

    /**
     * Идентификатор выплаты.
     *
     * @return the {@code uuid} field
     */
    public String uuid() {
        return uuid;
    }

    /** @return fields of the answer this SDK version does not know yet, exactly as received */
    public Map<String, Object> extra() {
        return extra;
    }

    /** @return a new, empty builder */
    public static Builder builder() {
        return new Builder();
    }

    /** @return a builder holding this object's values */
    public Builder toBuilder() {
        Builder builder = new Builder();
        builder.address = this.address;
        builder.amount = this.amount;
        builder.approvalRequired = this.approvalRequired;
        builder.commission = this.commission;
        builder.createdAt = this.createdAt;
        builder.currency = this.currency;
        builder.documentUrl = this.documentUrl;
        builder.error = this.error;
        builder.errorCode = this.errorCode;
        builder.feeBearer = this.feeBearer;
        builder.isFinal = this.isFinal;
        builder.isRefund = this.isRefund;
        builder.memo = this.memo;
        builder.network = this.network;
        builder.orderId = this.orderId;
        builder.payerAmount = this.payerAmount;
        builder.paymentOrderId = this.paymentOrderId;
        builder.refundFor = this.refundFor;
        builder.source = this.source;
        builder.status = this.status;
        builder.txid = this.txid;
        builder.updatedAt = this.updatedAt;
        builder.uuid = this.uuid;
        builder.nulls.addAll(this.nulls);
        builder.extra.putAll(this.extra);
        return builder;
    }

    /**
     * @param data the JSON object
     * @return the model
     * @throws IllegalArgumentException when a required field is missing or a value has the wrong type
     */
    public static PayoutInfoResult fromMap(Map<String, ?> data) {
        Builder builder = new Builder();
        builder.address = Wire.required(data, "address", Wire::string, "PayoutInfoResult");
        builder.amount = Wire.required(data, "amount", Wire::decimal, "PayoutInfoResult");
        builder.approvalRequired = Wire.required(
                data, "approval_required", Wire::bool, "PayoutInfoResult");
        builder.commission = Wire.required(data, "commission", Wire::decimal, "PayoutInfoResult");
        builder.createdAt = Wire.required(data, "created_at", Wire::string, "PayoutInfoResult");
        builder.currency = Wire.required(data, "currency", Wire::string, "PayoutInfoResult");
        builder.documentUrl = Wire.required(data, "document_url", Wire::string, "PayoutInfoResult");
        builder.error = Wire.optional(data, "error", Wire::string, "PayoutInfoResult");
        builder.errorCode = Wire.optional(data, "error_code", Wire::string, "PayoutInfoResult");
        builder.feeBearer = Wire.required(
                data, "fee_bearer", PayoutFeeBearer::fromJson, "PayoutInfoResult");
        builder.isFinal = Wire.required(data, "is_final", Wire::bool, "PayoutInfoResult");
        builder.isRefund = Wire.required(data, "is_refund", Wire::bool, "PayoutInfoResult");
        builder.memo = Wire.required(data, "memo", Wire::string, "PayoutInfoResult");
        builder.network = Wire.required(data, "network", Wire::string, "PayoutInfoResult");
        builder.orderId = Wire.optional(data, "order_id", Wire::string, "PayoutInfoResult");
        builder.payerAmount = Wire.required(
                data, "payer_amount", Wire::decimal, "PayoutInfoResult");
        builder.paymentOrderId = Wire.optional(
                data, "payment_order_id", Wire::string, "PayoutInfoResult");
        builder.refundFor = Wire.optional(data, "refund_for", Wire::string, "PayoutInfoResult");
        builder.source = Wire.required(data, "source", PayoutSource::fromJson, "PayoutInfoResult");
        builder.status = Wire.required(data, "status", PayoutStatus::fromJson, "PayoutInfoResult");
        builder.txid = Wire.required(data, "txid", Wire::string, "PayoutInfoResult");
        builder.updatedAt = Wire.required(data, "updated_at", Wire::string, "PayoutInfoResult");
        builder.uuid = Wire.required(data, "uuid", Wire::string, "PayoutInfoResult");
        builder.nulls.addAll(Wire.nulls(data, FIELDS));
        builder.extra.putAll(Wire.extra(data, FIELDS));
        return new PayoutInfoResult(builder);
    }

    /**
     * @param data a value of the JSON tree
     * @return the model
     * @throws IllegalArgumentException when it is not an object this model can read
     */
    public static PayoutInfoResult fromJson(Object data) {
        return fromMap(Wire.object(data, "PayoutInfoResult"));
    }

    /** @return the JSON object: set fields, explicit nulls and {@link #extra()} */
    public Map<String, Object> toMap() {
        Map<String, Object> out = new LinkedHashMap<>(this.extra);
        Wire.put(out, "address", this.address, this.nulls);
        Wire.put(out, "amount", this.amount, this.nulls);
        Wire.put(out, "approval_required", this.approvalRequired, this.nulls);
        Wire.put(out, "commission", this.commission, this.nulls);
        Wire.put(out, "created_at", this.createdAt, this.nulls);
        Wire.put(out, "currency", this.currency, this.nulls);
        Wire.put(out, "document_url", this.documentUrl, this.nulls);
        Wire.put(out, "error", this.error, this.nulls);
        Wire.put(out, "error_code", this.errorCode, this.nulls);
        Wire.put(out, "fee_bearer", this.feeBearer, this.nulls);
        Wire.put(out, "is_final", this.isFinal, this.nulls);
        Wire.put(out, "is_refund", this.isRefund, this.nulls);
        Wire.put(out, "memo", this.memo, this.nulls);
        Wire.put(out, "network", this.network, this.nulls);
        Wire.put(out, "order_id", this.orderId, this.nulls);
        Wire.put(out, "payer_amount", this.payerAmount, this.nulls);
        Wire.put(out, "payment_order_id", this.paymentOrderId, this.nulls);
        Wire.put(out, "refund_for", this.refundFor, this.nulls);
        Wire.put(out, "source", this.source, this.nulls);
        Wire.put(out, "status", this.status, this.nulls);
        Wire.put(out, "txid", this.txid, this.nulls);
        Wire.put(out, "updated_at", this.updatedAt, this.nulls);
        Wire.put(out, "uuid", this.uuid, this.nulls);
        return out;
    }

    @Override
    public Object toJson() {
        return toMap();
    }

    @Override
    public boolean equals(Object other) {
        return other instanceof PayoutInfoResult that
                && Objects.equals(this.address, that.address)
                && Objects.equals(this.amount, that.amount)
                && Objects.equals(this.approvalRequired, that.approvalRequired)
                && Objects.equals(this.commission, that.commission)
                && Objects.equals(this.createdAt, that.createdAt)
                && Objects.equals(this.currency, that.currency)
                && Objects.equals(this.documentUrl, that.documentUrl)
                && Objects.equals(this.error, that.error)
                && Objects.equals(this.errorCode, that.errorCode)
                && Objects.equals(this.feeBearer, that.feeBearer)
                && Objects.equals(this.isFinal, that.isFinal)
                && Objects.equals(this.isRefund, that.isRefund)
                && Objects.equals(this.memo, that.memo)
                && Objects.equals(this.network, that.network)
                && Objects.equals(this.orderId, that.orderId)
                && Objects.equals(this.payerAmount, that.payerAmount)
                && Objects.equals(this.paymentOrderId, that.paymentOrderId)
                && Objects.equals(this.refundFor, that.refundFor)
                && Objects.equals(this.source, that.source)
                && Objects.equals(this.status, that.status)
                && Objects.equals(this.txid, that.txid)
                && Objects.equals(this.updatedAt, that.updatedAt)
                && Objects.equals(this.uuid, that.uuid)
                && this.nulls.equals(that.nulls)
                && this.extra.equals(that.extra);
    }

    @Override
    public int hashCode() {
        return Objects.hash(
                this.address,
                this.amount,
                this.approvalRequired,
                this.commission,
                this.createdAt,
                this.currency,
                this.documentUrl,
                this.error,
                this.errorCode,
                this.feeBearer,
                this.isFinal,
                this.isRefund,
                this.memo,
                this.network,
                this.orderId,
                this.payerAmount,
                this.paymentOrderId,
                this.refundFor,
                this.source,
                this.status,
                this.txid,
                this.updatedAt,
                this.uuid,
                this.nulls,
                this.extra);
    }

    @Override
    public String toString() {
        return Wire.describe(
                "PayoutInfoResult",
                "address",
                this.address,
                "amount",
                this.amount,
                "approvalRequired",
                this.approvalRequired,
                "commission",
                this.commission,
                "createdAt",
                this.createdAt,
                "currency",
                this.currency,
                "documentUrl",
                this.documentUrl,
                "error",
                this.error,
                "errorCode",
                this.errorCode,
                "feeBearer",
                this.feeBearer,
                "isFinal",
                this.isFinal,
                "isRefund",
                this.isRefund,
                "memo",
                this.memo,
                "network",
                this.network,
                "orderId",
                this.orderId,
                "payerAmount",
                this.payerAmount,
                "paymentOrderId",
                this.paymentOrderId,
                "refundFor",
                this.refundFor,
                "source",
                this.source,
                "status",
                this.status,
                "txid",
                this.txid,
                "updatedAt",
                this.updatedAt,
                "uuid",
                this.uuid,
                "extra",
                this.extra);
    }

    /** Builds a {@link PayoutInfoResult}. */
    public static final class Builder {
        private String address;
        private BigDecimal amount;
        private Boolean approvalRequired;
        private BigDecimal commission;
        private String createdAt;
        private String currency;
        private String documentUrl;
        private String error;
        private String errorCode;
        private PayoutFeeBearer feeBearer;
        private Boolean isFinal;
        private Boolean isRefund;
        private String memo;
        private String network;
        private String orderId;
        private BigDecimal payerAmount;
        private String paymentOrderId;
        private String refundFor;
        private PayoutSource source;
        private PayoutStatus status;
        private String txid;
        private String updatedAt;
        private String uuid;
        private final Set<String> nulls = new LinkedHashSet<>();
        private final Map<String, Object> extra = new LinkedHashMap<>();

        private Builder() {}

        /**
         * Sets {@code address}.
         *
         * <p>Адрес получателя.
         *
         * @param address the value
         * @return this builder
         */
        public Builder address(String address) {
            this.address = address;
            return this;
        }

        /**
         * Sets {@code amount}.
         *
         * <p>Сумма выплаты в валюте currency, списанная с вашего баланса.
         *
         * @param amount the value
         * @return this builder
         */
        public Builder amount(BigDecimal amount) {
            this.amount = amount;
            return this;
        }

        /**
         * Sets {@code amount}.
         *
         * <p>Сумма выплаты в валюте currency, списанная с вашего баланса.
         *
         * @param amount the value as a decimal string, such as {@code "10.50"}
         * @return this builder
         */
        public Builder amount(String amount) {
            return amount(amount == null ? null : new BigDecimal(amount));
        }

        /**
         * Sets {@code approval_required}.
         *
         * <p>true — выплата ждёт подтверждения (внутренние сценарии; по API-ключу всегда false).
         *
         * @param approvalRequired the value
         * @return this builder
         */
        public Builder approvalRequired(Boolean approvalRequired) {
            this.approvalRequired = approvalRequired;
            return this;
        }

        /**
         * Sets {@code commission}.
         *
         * <p>Удержанная сетевая комиссия, в валюте выплаты. 0 — комиссию поглотил шлюз.
         *
         * @param commission the value
         * @return this builder
         */
        public Builder commission(BigDecimal commission) {
            this.commission = commission;
            return this;
        }

        /**
         * Sets {@code commission}.
         *
         * <p>Удержанная сетевая комиссия, в валюте выплаты. 0 — комиссию поглотил шлюз.
         *
         * @param commission the value as a decimal string, such as {@code "10.50"}
         * @return this builder
         */
        public Builder commission(String commission) {
            return commission(commission == null ? null : new BigDecimal(commission));
        }

        /**
         * Sets {@code created_at}.
         *
         * <p>Время создания (ISO 8601).
         *
         * @param createdAt the value
         * @return this builder
         */
        public Builder createdAt(String createdAt) {
            this.createdAt = createdAt;
            return this;
        }

        /**
         * Sets {@code currency}.
         *
         * <p>Код валюты выплаты.
         *
         * @param currency the value
         * @return this builder
         */
        public Builder currency(String currency) {
            this.currency = currency;
            return this;
        }

        /**
         * Sets {@code document_url}.
         *
         * <p>Подписанная ссылка на PDF-чек этой операции — открывается без API-ключа, можно вложить
         * в письмо или отдать получателю. Пусто, если генерация документов не включена.
         *
         * @param documentUrl the value
         * @return this builder
         */
        public Builder documentUrl(String documentUrl) {
            this.documentUrl = documentUrl;
            return this;
        }

        /**
         * Sets {@code error}.
         *
         * <p>Причина сбоя выплаты человеческим текстом; null — сбоя нет.
         *
         * @param error the value
         * @return this builder
         */
        public Builder error(String error) {
            this.error = error;
            Wire.mark(this.nulls, "error", error);
            return this;
        }

        /**
         * Sets {@code error_code}.
         *
         * <p>Машинный код причины; null — сбоя нет.
         *
         * @param errorCode the value
         * @return this builder
         */
        public Builder errorCode(String errorCode) {
            this.errorCode = errorCode;
            Wire.mark(this.nulls, "error_code", errorCode);
            return this;
        }

        /**
         * Sets {@code fee_bearer}.
         *
         * <p>Кто заплатил сетевую комиссию: gateway — шлюз поглотил её (commission = 0); merchant —
         * сумма списания увеличена на комиссию, получатель получает запрошенное целиком
         * (is_subtract=true, выплатная ссылка с fee_bearer=merchant); recipient — комиссия удержана
         * из выплаты, получателю приходит меньше запрошенного.
         *
         * @param feeBearer the value
         * @return this builder
         */
        public Builder feeBearer(PayoutFeeBearer feeBearer) {
            this.feeBearer = feeBearer;
            return this;
        }

        /**
         * Sets {@code fee_bearer}.
         *
         * <p>Кто заплатил сетевую комиссию: gateway — шлюз поглотил её (commission = 0); merchant —
         * сумма списания увеличена на комиссию, получатель получает запрошенное целиком
         * (is_subtract=true, выплатная ссылка с fee_bearer=merchant); recipient — комиссия удержана
         * из выплаты, получателю приходит меньше запрошенного.
         *
         * @param feeBearer the value as the API sends it; one this SDK version does not know is
         *     accepted
         * @return this builder
         */
        public Builder feeBearer(String feeBearer) {
            return feeBearer(feeBearer == null ? null : PayoutFeeBearer.of(feeBearer));
        }

        /**
         * Sets {@code is_final}.
         *
         * <p>true — статус финальный (confirmed / failed / cancelled).
         *
         * @param isFinal the value
         * @return this builder
         */
        public Builder isFinal(Boolean isFinal) {
            this.isFinal = isFinal;
            return this;
        }

        /**
         * Sets {@code is_refund}.
         *
         * <p>true — это возврат платежа, а не обычная выплата.
         *
         * @param isRefund the value
         * @return this builder
         */
        public Builder isRefund(Boolean isRefund) {
            this.isRefund = isRefund;
            return this;
        }

        /**
         * Sets {@code memo}.
         *
         * <p>Тег/мемо назначения, переданный при создании (TON Jetton, memo-биржи). Пусто — без
         * мемо.
         *
         * @param memo the value
         * @return this builder
         */
        public Builder memo(String memo) {
            this.memo = memo;
            return this;
        }

        /**
         * Sets {@code network}.
         *
         * <p>Сеть блокчейна.
         *
         * @param network the value
         * @return this builder
         */
        public Builder network(String network) {
            this.network = network;
            return this;
        }

        /**
         * Sets {@code order_id}.
         *
         * <p>Ваш номер (reference) выплаты. У возврата — null: возврат не имеет вашего
         * идентификатора, см. payment_order_id.
         *
         * @param orderId the value
         * @return this builder
         */
        public Builder orderId(String orderId) {
            this.orderId = orderId;
            Wire.mark(this.nulls, "order_id", orderId);
            return this;
        }

        /**
         * Sets {@code payer_amount}.
         *
         * <p>Сколько реально уходит получателю на адрес: amount − commission.
         *
         * @param payerAmount the value
         * @return this builder
         */
        public Builder payerAmount(BigDecimal payerAmount) {
            this.payerAmount = payerAmount;
            return this;
        }

        /**
         * Sets {@code payer_amount}.
         *
         * <p>Сколько реально уходит получателю на адрес: amount − commission.
         *
         * @param payerAmount the value as a decimal string, such as {@code "10.50"}
         * @return this builder
         */
        public Builder payerAmount(String payerAmount) {
            return payerAmount(payerAmount == null ? null : new BigDecimal(payerAmount));
        }

        /**
         * Sets {@code payment_order_id}.
         *
         * <p>Ваш order_id платежа, по которому сделан возврат (null у обычной выплаты). У возврата
         * собственного order_id нет — он приходит null, а сверять возврат с заказом нужно по этому
         * полю.
         *
         * @param paymentOrderId the value
         * @return this builder
         */
        public Builder paymentOrderId(String paymentOrderId) {
            this.paymentOrderId = paymentOrderId;
            Wire.mark(this.nulls, "payment_order_id", paymentOrderId);
            return this;
        }

        /**
         * Sets {@code refund_for}.
         *
         * <p>Идентификатор возвращаемого платежа (null, если это не возврат).
         *
         * @param refundFor the value
         * @return this builder
         */
        public Builder refundFor(String refundFor) {
            this.refundFor = refundFor;
            Wire.mark(this.nulls, "refund_for", refundFor);
            return this;
        }

        /**
         * Sets {@code source}.
         *
         * <p>api (через интеграцию) | manual (из кабинета).
         *
         * @param source the value
         * @return this builder
         */
        public Builder source(PayoutSource source) {
            this.source = source;
            return this;
        }

        /**
         * Sets {@code source}.
         *
         * <p>api (через интеграцию) | manual (из кабинета).
         *
         * @param source the value as the API sends it; one this SDK version does not know is
         *     accepted
         * @return this builder
         */
        public Builder source(String source) {
            return source(source == null ? null : PayoutSource.of(source));
        }

        /**
         * Sets {@code status}.
         *
         * <p>Статус выплаты: pending (создана, ждёт) | approved (одобрена) | awaiting_cosign (ждёт
         * второй подписи) | broadcasting (отправляется) | sent (отправлена, ждёт подтверждений) |
         * confirmed (подтверждена — готово) | failed | cancelled. Значение можно передать обратно в
         * фильтр истории как есть.
         *
         * @param status the value
         * @return this builder
         */
        public Builder status(PayoutStatus status) {
            this.status = status;
            return this;
        }

        /**
         * Sets {@code status}.
         *
         * <p>Статус выплаты: pending (создана, ждёт) | approved (одобрена) | awaiting_cosign (ждёт
         * второй подписи) | broadcasting (отправляется) | sent (отправлена, ждёт подтверждений) |
         * confirmed (подтверждена — готово) | failed | cancelled. Значение можно передать обратно в
         * фильтр истории как есть.
         *
         * @param status the value as the API sends it; one this SDK version does not know is
         *     accepted
         * @return this builder
         */
        public Builder status(String status) {
            return status(status == null ? null : PayoutStatus.of(status));
        }

        /**
         * Sets {@code txid}.
         *
         * <p>Хеш транзакции в блокчейне (появляется после отправки).
         *
         * @param txid the value
         * @return this builder
         */
        public Builder txid(String txid) {
            this.txid = txid;
            return this;
        }

        /**
         * Sets {@code updated_at}.
         *
         * <p>Время последнего изменения (ISO 8601).
         *
         * @param updatedAt the value
         * @return this builder
         */
        public Builder updatedAt(String updatedAt) {
            this.updatedAt = updatedAt;
            return this;
        }

        /**
         * Sets {@code uuid}.
         *
         * <p>Идентификатор выплаты.
         *
         * @param uuid the value
         * @return this builder
         */
        public Builder uuid(String uuid) {
            this.uuid = uuid;
            return this;
        }

        /**
         * A field this SDK version does not know yet, sent as given.
         *
         * @param name the field's JSON name
         * @param value its value: a JSON tree, a model, an enum or a BigDecimal
         * @return this builder
         */
        public Builder putExtra(String name, Object value) {
            this.extra.put(name, value);
            return this;
        }

        /**
         * @return the {@link PayoutInfoResult}
         * @throws IllegalStateException when a required field is not set
         */
        public PayoutInfoResult build() {
            Wire.require(this.address, "PayoutInfoResult", "address");
            Wire.require(this.amount, "PayoutInfoResult", "amount");
            Wire.require(this.approvalRequired, "PayoutInfoResult", "approval_required");
            Wire.require(this.commission, "PayoutInfoResult", "commission");
            Wire.require(this.createdAt, "PayoutInfoResult", "created_at");
            Wire.require(this.currency, "PayoutInfoResult", "currency");
            Wire.require(this.documentUrl, "PayoutInfoResult", "document_url");
            Wire.require(this.feeBearer, "PayoutInfoResult", "fee_bearer");
            Wire.require(this.isFinal, "PayoutInfoResult", "is_final");
            Wire.require(this.isRefund, "PayoutInfoResult", "is_refund");
            Wire.require(this.memo, "PayoutInfoResult", "memo");
            Wire.require(this.network, "PayoutInfoResult", "network");
            Wire.require(this.payerAmount, "PayoutInfoResult", "payer_amount");
            Wire.require(this.source, "PayoutInfoResult", "source");
            Wire.require(this.status, "PayoutInfoResult", "status");
            Wire.require(this.txid, "PayoutInfoResult", "txid");
            Wire.require(this.updatedAt, "PayoutInfoResult", "updated_at");
            Wire.require(this.uuid, "PayoutInfoResult", "uuid");
            return new PayoutInfoResult(this);
        }
    }
}
