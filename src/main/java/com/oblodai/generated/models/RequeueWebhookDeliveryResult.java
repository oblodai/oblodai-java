// Generated by oblodai-backend tools/sdkgen from openapi.json - do not edit.

package com.oblodai.generated.models;

import java.util.LinkedHashMap;
import java.util.LinkedHashSet;
import java.util.Map;
import java.util.Objects;
import java.util.Set;

/**
 * Build one with {@link #builder()}; fields the SDK does not know yet are kept in {@link #extra()}
 * and sent back as they came.
 */
public final class RequeueWebhookDeliveryResult implements WireObject {

    private static final Set<String> FIELDS = Set.of(
            "id",
            "ok",
            "status");

    private final String id;
    private final Boolean ok;
    private final WebhookDeliveryStatus status;
    private final Set<String> nulls;
    private final Map<String, Object> extra;

    private RequeueWebhookDeliveryResult(Builder builder) {
        this.id = builder.id;
        this.ok = builder.ok;
        this.status = builder.status;
        this.nulls = Set.copyOf(builder.nulls);
        this.extra = Wire.copyMap(builder.extra);
    }

    /**
     * Delivery id.
     *
     * @return the {@code id} field
     */
    public String id() {
        return id;
    }

    /**
     * true — this call re-queued the delivery; false — it was already queued or delivered
     * (repeating the call changes nothing).
     *
     * @return the {@code ok} field
     */
    public Boolean ok() {
        return ok;
    }

    /**
     * The delivery status after the call: pending — queued again; delivered — already delivered,
     * nothing to repeat.
     *
     * @return the {@code status} field
     */
    public WebhookDeliveryStatus status() {
        return status;
    }

    /** @return fields of the answer this SDK version does not know yet, exactly as received */
    public Map<String, Object> extra() {
        return extra;
    }

    /** @return a new, empty builder */
    public static Builder builder() {
        return new Builder();
    }

    /** @return a builder holding this object's values */
    public Builder toBuilder() {
        Builder builder = new Builder();
        builder.id = this.id;
        builder.ok = this.ok;
        builder.status = this.status;
        builder.nulls.addAll(this.nulls);
        builder.extra.putAll(this.extra);
        return builder;
    }

    /**
     * @param data the JSON object
     * @return the model
     * @throws IllegalArgumentException when a required field is missing or a value has the wrong type
     */
    public static RequeueWebhookDeliveryResult fromMap(Map<String, ?> data) {
        Builder builder = new Builder();
        builder.id = Wire.required(data, "id", Wire::string, "RequeueWebhookDeliveryResult");
        builder.ok = Wire.required(data, "ok", Wire::bool, "RequeueWebhookDeliveryResult");
        builder.status = Wire.required(
                data, "status", WebhookDeliveryStatus::fromJson, "RequeueWebhookDeliveryResult");
        builder.nulls.addAll(Wire.nulls(data, FIELDS));
        builder.extra.putAll(Wire.extra(data, FIELDS));
        return new RequeueWebhookDeliveryResult(builder);
    }

    /**
     * @param data a value of the JSON tree
     * @return the model
     * @throws IllegalArgumentException when it is not an object this model can read
     */
    public static RequeueWebhookDeliveryResult fromJson(Object data) {
        return fromMap(Wire.object(data, "RequeueWebhookDeliveryResult"));
    }

    /** @return the JSON object: set fields, explicit nulls and {@link #extra()} */
    public Map<String, Object> toMap() {
        Map<String, Object> out = new LinkedHashMap<>(this.extra);
        Wire.put(out, "id", this.id, this.nulls);
        Wire.put(out, "ok", this.ok, this.nulls);
        Wire.put(out, "status", this.status, this.nulls);
        return out;
    }

    @Override
    public Object toJson() {
        return toMap();
    }

    @Override
    public boolean equals(Object other) {
        return other instanceof RequeueWebhookDeliveryResult that
                && Objects.equals(this.id, that.id)
                && Objects.equals(this.ok, that.ok)
                && Objects.equals(this.status, that.status)
                && this.nulls.equals(that.nulls)
                && this.extra.equals(that.extra);
    }

    @Override
    public int hashCode() {
        return Objects.hash(
                this.id,
                this.ok,
                this.status,
                this.nulls,
                this.extra);
    }

    @Override
    public String toString() {
        return Wire.describe(
                "RequeueWebhookDeliveryResult",
                "id",
                this.id,
                "ok",
                this.ok,
                "status",
                this.status,
                "extra",
                this.extra);
    }

    /** Builds a {@link RequeueWebhookDeliveryResult}. */
    public static final class Builder {
        private String id;
        private Boolean ok;
        private WebhookDeliveryStatus status;
        private final Set<String> nulls = new LinkedHashSet<>();
        private final Map<String, Object> extra = new LinkedHashMap<>();

        private Builder() {}

        /**
         * Sets {@code id}.
         *
         * <p>Delivery id.
         *
         * @param id the value
         * @return this builder
         */
        public Builder id(String id) {
            this.id = id;
            return this;
        }

        /**
         * Sets {@code ok}.
         *
         * <p>true — this call re-queued the delivery; false — it was already queued or delivered
         * (repeating the call changes nothing).
         *
         * @param ok the value
         * @return this builder
         */
        public Builder ok(Boolean ok) {
            this.ok = ok;
            return this;
        }

        /**
         * Sets {@code status}.
         *
         * <p>The delivery status after the call: pending — queued again; delivered — already
         * delivered, nothing to repeat.
         *
         * @param status the value
         * @return this builder
         */
        public Builder status(WebhookDeliveryStatus status) {
            this.status = status;
            return this;
        }

        /**
         * Sets {@code status}.
         *
         * <p>The delivery status after the call: pending — queued again; delivered — already
         * delivered, nothing to repeat.
         *
         * @param status the value as the API sends it; one this SDK version does not know is
         *     accepted
         * @return this builder
         */
        public Builder status(String status) {
            return status(status == null ? null : WebhookDeliveryStatus.of(status));
        }

        /**
         * A field this SDK version does not know yet, sent as given.
         *
         * @param name the field's JSON name
         * @param value its value: a JSON tree, a model, an enum or a BigDecimal
         * @return this builder
         */
        public Builder putExtra(String name, Object value) {
            this.extra.put(name, value);
            return this;
        }

        /**
         * @return the {@link RequeueWebhookDeliveryResult}
         * @throws IllegalStateException when a required field is not set
         */
        public RequeueWebhookDeliveryResult build() {
            Wire.require(this.id, "RequeueWebhookDeliveryResult", "id");
            Wire.require(this.ok, "RequeueWebhookDeliveryResult", "ok");
            Wire.require(this.status, "RequeueWebhookDeliveryResult", "status");
            return new RequeueWebhookDeliveryResult(this);
        }
    }
}
