// Generated by oblodai-backend tools/sdkgen from openapi.json - do not edit.

package com.oblodai.generated.models;

import java.util.LinkedHashMap;
import java.util.LinkedHashSet;
import java.util.Map;
import java.util.Objects;
import java.util.Set;

/**
 * Build one with {@link #builder()}; fields the SDK does not know yet are kept in {@link #extra()}
 * and sent back as they came.
 */
public final class PaymentDiscountRule implements WireObject {

    private static final Set<String> FIELDS = Set.of(
            "currency",
            "discount_percent",
            "network");

    private final String currency;
    private final Long discountPercent;
    private final String network;
    private final Set<String> nulls;
    private final Map<String, Object> extra;

    private PaymentDiscountRule(Builder builder) {
        this.currency = builder.currency;
        this.discountPercent = builder.discountPercent;
        this.network = builder.network;
        this.nulls = Set.copyOf(builder.nulls);
        this.extra = Wire.copyMap(builder.extra);
    }

    /**
     * Монета правила. Пусто — правило по умолчанию для всех монет, у которых нет своего.
     *
     * @return the {@code currency} field
     */
    public String currency() {
        return currency;
    }

    /**
     * Процент, от -99 до 99. Плюс — скидка плательщику за оплату этой монетой, минус — наценка.
     *
     * @return the {@code discount_percent} field
     */
    public Long discountPercent() {
        return discountPercent;
    }

    /**
     * Сеть. Пусто — любая сеть этой монеты.
     *
     * @return the {@code network} field
     */
    public String network() {
        return network;
    }

    /** @return fields of the answer this SDK version does not know yet, exactly as received */
    public Map<String, Object> extra() {
        return extra;
    }

    /** @return a new, empty builder */
    public static Builder builder() {
        return new Builder();
    }

    /** @return a builder holding this object's values */
    public Builder toBuilder() {
        Builder builder = new Builder();
        builder.currency = this.currency;
        builder.discountPercent = this.discountPercent;
        builder.network = this.network;
        builder.nulls.addAll(this.nulls);
        builder.extra.putAll(this.extra);
        return builder;
    }

    /**
     * @param data the JSON object
     * @return the model
     * @throws IllegalArgumentException when a required field is missing or a value has the wrong type
     */
    public static PaymentDiscountRule fromMap(Map<String, ?> data) {
        Builder builder = new Builder();
        builder.currency = Wire.required(data, "currency", Wire::string, "PaymentDiscountRule");
        builder.discountPercent = Wire.required(
                data, "discount_percent", Wire::integer, "PaymentDiscountRule");
        builder.network = Wire.required(data, "network", Wire::string, "PaymentDiscountRule");
        builder.nulls.addAll(Wire.nulls(data, FIELDS));
        builder.extra.putAll(Wire.extra(data, FIELDS));
        return new PaymentDiscountRule(builder);
    }

    /**
     * @param data a value of the JSON tree
     * @return the model
     * @throws IllegalArgumentException when it is not an object this model can read
     */
    public static PaymentDiscountRule fromJson(Object data) {
        return fromMap(Wire.object(data, "PaymentDiscountRule"));
    }

    /** @return the JSON object: set fields, explicit nulls and {@link #extra()} */
    public Map<String, Object> toMap() {
        Map<String, Object> out = new LinkedHashMap<>(this.extra);
        Wire.put(out, "currency", this.currency, this.nulls);
        Wire.put(out, "discount_percent", this.discountPercent, this.nulls);
        Wire.put(out, "network", this.network, this.nulls);
        return out;
    }

    @Override
    public Object toJson() {
        return toMap();
    }

    @Override
    public boolean equals(Object other) {
        return other instanceof PaymentDiscountRule that
                && Objects.equals(this.currency, that.currency)
                && Objects.equals(this.discountPercent, that.discountPercent)
                && Objects.equals(this.network, that.network)
                && this.nulls.equals(that.nulls)
                && this.extra.equals(that.extra);
    }

    @Override
    public int hashCode() {
        return Objects.hash(
                this.currency,
                this.discountPercent,
                this.network,
                this.nulls,
                this.extra);
    }

    @Override
    public String toString() {
        return Wire.describe(
                "PaymentDiscountRule",
                "currency",
                this.currency,
                "discountPercent",
                this.discountPercent,
                "network",
                this.network,
                "extra",
                this.extra);
    }

    /** Builds a {@link PaymentDiscountRule}. */
    public static final class Builder {
        private String currency;
        private Long discountPercent;
        private String network;
        private final Set<String> nulls = new LinkedHashSet<>();
        private final Map<String, Object> extra = new LinkedHashMap<>();

        private Builder() {}

        /**
         * Sets {@code currency}.
         *
         * <p>Монета правила. Пусто — правило по умолчанию для всех монет, у которых нет своего.
         *
         * @param currency the value
         * @return this builder
         */
        public Builder currency(String currency) {
            this.currency = currency;
            return this;
        }

        /**
         * Sets {@code discount_percent}.
         *
         * <p>Процент, от -99 до 99. Плюс — скидка плательщику за оплату этой монетой, минус —
         * наценка.
         *
         * @param discountPercent the value
         * @return this builder
         */
        public Builder discountPercent(Long discountPercent) {
            this.discountPercent = discountPercent;
            return this;
        }

        /**
         * Sets {@code network}.
         *
         * <p>Сеть. Пусто — любая сеть этой монеты.
         *
         * @param network the value
         * @return this builder
         */
        public Builder network(String network) {
            this.network = network;
            return this;
        }

        /**
         * A field this SDK version does not know yet, sent as given.
         *
         * @param name the field's JSON name
         * @param value its value: a JSON tree, a model, an enum or a BigDecimal
         * @return this builder
         */
        public Builder putExtra(String name, Object value) {
            this.extra.put(name, value);
            return this;
        }

        /**
         * @return the {@link PaymentDiscountRule}
         * @throws IllegalStateException when a required field is not set
         */
        public PaymentDiscountRule build() {
            Wire.require(this.currency, "PaymentDiscountRule", "currency");
            Wire.require(this.discountPercent, "PaymentDiscountRule", "discount_percent");
            Wire.require(this.network, "PaymentDiscountRule", "network");
            return new PaymentDiscountRule(this);
        }
    }
}
