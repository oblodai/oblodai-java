// Generated by oblodai-backend tools/sdkgen from openapi.json - do not edit.

package com.oblodai.generated.models;

import java.util.LinkedHashMap;
import java.util.LinkedHashSet;
import java.util.Map;
import java.util.Objects;
import java.util.Set;

/**
 * Build one with {@link #builder()}; fields the SDK does not know yet are kept in {@link #extra()}
 * and sent back as they came.
 */
public final class ClaimRequest implements WireObject {

    private static final Set<String> FIELDS = Set.of(
            "address",
            "memo",
            "passcode");

    private final String address;
    private final String memo;
    private final String passcode;
    private final Set<String> nulls;
    private final Map<String, Object> extra;

    private ClaimRequest(Builder builder) {
        this.address = builder.address;
        this.memo = builder.memo;
        this.passcode = builder.passcode;
        this.nulls = Set.copyOf(builder.nulls);
        this.extra = Wire.copyMap(builder.extra);
    }

    /**
     * The recipient's address on the payout network.
     *
     * @return the {@code address} field
     */
    public String address() {
        return address;
    }

    /**
     * Memo/tag — only for networks where it is required.
     *
     * @return the {@code memo} field, or {@code null} when absent
     */
    public String memo() {
        return memo;
    }

    /**
     * Claim passcode — if the sender set one on the link. After 10 wrong attempts the link is
     * locked.
     *
     * @return the {@code passcode} field, or {@code null} when absent
     */
    public String passcode() {
        return passcode;
    }

    /** @return fields of the answer this SDK version does not know yet, exactly as received */
    public Map<String, Object> extra() {
        return extra;
    }

    /** @return a new, empty builder */
    public static Builder builder() {
        return new Builder();
    }

    /** @return a builder holding this object's values */
    public Builder toBuilder() {
        Builder builder = new Builder();
        builder.address = this.address;
        builder.memo = this.memo;
        builder.passcode = this.passcode;
        builder.nulls.addAll(this.nulls);
        builder.extra.putAll(this.extra);
        return builder;
    }

    /**
     * @param data the JSON object
     * @return the model
     * @throws IllegalArgumentException when a required field is missing or a value has the wrong type
     */
    public static ClaimRequest fromMap(Map<String, ?> data) {
        Builder builder = new Builder();
        builder.address = Wire.required(data, "address", Wire::string, "ClaimRequest");
        builder.memo = Wire.optional(data, "memo", Wire::string, "ClaimRequest");
        builder.passcode = Wire.optional(data, "passcode", Wire::string, "ClaimRequest");
        builder.nulls.addAll(Wire.nulls(data, FIELDS));
        builder.extra.putAll(Wire.extra(data, FIELDS));
        return new ClaimRequest(builder);
    }

    /**
     * @param data a value of the JSON tree
     * @return the model
     * @throws IllegalArgumentException when it is not an object this model can read
     */
    public static ClaimRequest fromJson(Object data) {
        return fromMap(Wire.object(data, "ClaimRequest"));
    }

    /** @return the JSON object: set fields, explicit nulls and {@link #extra()} */
    public Map<String, Object> toMap() {
        Map<String, Object> out = new LinkedHashMap<>(this.extra);
        Wire.put(out, "address", this.address, this.nulls);
        Wire.put(out, "memo", this.memo, this.nulls);
        Wire.put(out, "passcode", this.passcode, this.nulls);
        return out;
    }

    @Override
    public Object toJson() {
        return toMap();
    }

    @Override
    public boolean equals(Object other) {
        return other instanceof ClaimRequest that
                && Objects.equals(this.address, that.address)
                && Objects.equals(this.memo, that.memo)
                && Objects.equals(this.passcode, that.passcode)
                && this.nulls.equals(that.nulls)
                && this.extra.equals(that.extra);
    }

    @Override
    public int hashCode() {
        return Objects.hash(
                this.address,
                this.memo,
                this.passcode,
                this.nulls,
                this.extra);
    }

    @Override
    public String toString() {
        return Wire.describe(
                "ClaimRequest",
                "address",
                this.address,
                "memo",
                this.memo,
                "passcode",
                this.passcode,
                "extra",
                this.extra);
    }

    /** Builds a {@link ClaimRequest}. */
    public static final class Builder {
        private String address;
        private String memo;
        private String passcode;
        private final Set<String> nulls = new LinkedHashSet<>();
        private final Map<String, Object> extra = new LinkedHashMap<>();

        private Builder() {}

        /**
         * Sets {@code address}.
         *
         * <p>The recipient's address on the payout network.
         *
         * @param address the value
         * @return this builder
         */
        public Builder address(String address) {
            this.address = address;
            return this;
        }

        /**
         * Sets {@code memo}.
         *
         * <p>Memo/tag — only for networks where it is required.
         *
         * @param memo the value
         * @return this builder
         */
        public Builder memo(String memo) {
            this.memo = memo;
            return this;
        }

        /**
         * Sets {@code passcode}.
         *
         * <p>Claim passcode — if the sender set one on the link. After 10 wrong attempts the link
         * is locked.
         *
         * @param passcode the value
         * @return this builder
         */
        public Builder passcode(String passcode) {
            this.passcode = passcode;
            return this;
        }

        /**
         * A field this SDK version does not know yet, sent as given.
         *
         * @param name the field's JSON name
         * @param value its value: a JSON tree, a model, an enum or a BigDecimal
         * @return this builder
         */
        public Builder putExtra(String name, Object value) {
            this.extra.put(name, value);
            return this;
        }

        /**
         * @return the {@link ClaimRequest}
         * @throws IllegalStateException when a required field is not set
         */
        public ClaimRequest build() {
            Wire.require(this.address, "ClaimRequest", "address");
            return new ClaimRequest(this);
        }
    }
}
