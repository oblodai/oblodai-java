 ...
 * @param id {@value SigningProtocol#WEBHOOK_HEADER_ID} - stable across retries of the same
 *     DELIVERY; a resend of the same state is a new delivery with a new id, so deduplicate on
 *     {@code eventId}
 * @param eventId {@value SigningProtocol#WEBHOOK_HEADER_EVENT_ID} - the id of the STATE this
 ...
 * @param eventType {@value SigningProtocol#WEBHOOK_HEADER_EVENT} — the event name, one of the
 *     events of a kind in {@link com.oblodai.generated.Facts#WEBHOOK_KINDS}
 * @param eventTime {@value SigningProtocol#WEBHOOK_HEADER_EVENT_TIME} — unix seconds when the state
 *     change committed
 * @param sentAt {@value SigningProtocol#WEBHOOK_HEADER_TIMESTAMP} — unix seconds when this attempt
 *     was sent
